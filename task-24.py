...
yagod = list(random.randint(0, 10) for i in range(kust))
result = []
i = 0
sum = 0
...
while (i < kust):
    if (i == kust - 1):
        sum = yagod[i] + yagod[i - 1] + yagod[0]
    else:
        sum = yagod[i] + yagod[i - 1] + yagod[i + 1]
        result.append(sum)
        result.sort()
    i += 1

print(f"Макс. число ягод за один заход: {result[-1]}")
